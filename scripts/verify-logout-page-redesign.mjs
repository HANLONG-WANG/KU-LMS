...
import { read, readKulmsSource, extractFunction, escapeRegex, assert } from './lib/content-source.mjs';
...
record('logout view parser and renderer exist', () => {
  for (const token of ['function buildLogoutView(', 'function parseLogoutView(', 'function renderLogout(', 'function findShortestMatchingText(']) {
    assert(source.includes(token), `Missing logout token: ${token}`);
  }
});
record('logout action parity is preserved in code', () => {
  assert(source.includes("includes('/webclass/login.php')"), 'Logout parser no longer captures the native login-return action.');
  assert(source.includes('このウィンドウを閉じる'), 'Logout parser no longer captures the native close-window action label.');
  assert(source.includes("throw new Error('Logout actions not found')"), 'Logout view should fail open when required terminal actions are missing.');
});
record('logout shell stays unauthenticated', () => {
  assert(source.includes("route.name === 'login' || route.name === 'logout'"), 'Logout route is not using the auth-terminal shell branch.');
  assert(source.includes("const pageClass = route.name === 'logout' ? 'ku-logout-page' : 'ku-login-page';"), 'Logout route pageClass contract is missing.');
  assert(source.includes('<section class="ku-card ku-logout-warning-card">'), 'Logout warning card markup is missing.');
  assert(source.includes("ku-logout-shell ${warningCard ? 'has-warning' : 'is-compact'}"), 'Logout compact-vs-warning shell contract is missing.');
});
record('refresh logic still fails closed on logout during active refresh', () => {
  assert(source.includes("route.name === 'login' || route.name === 'logout'"), 'Refresh continuation is missing the logout fail-closed branch.');
...
record('logout dedupe regression is locked by parser and renderer', () => {
  const sandbox = {
    ...
    absoluteUrl(path = '') { if (!path) return ''; if (/^https?:/i.test(path) || String(path).startsWith('javascript:')) return String(path); return new URL(String(path), 'https://kulms.tl.kansai-u.ac.jp').toString(); },
  ...
  for (const fn of ['findShortestMatchingText', 'extractFirstMatch', 'parseLogoutView', 'renderLogout']) {
    vm.runInContext(extractFunction(source, fn), sandbox, { filename: 'kulms-source.js' });
  }
  const textNodes = [
  ...
  const anchors = [
    { textContent: 'ログイン画面に戻る', getAttribute(name) { return name === 'href' ? '/webclass/login.php?language=JAPANESE' : ''; } },
    { textContent: 'このウィンドウを閉じる', getAttribute(name) { return name === 'href' ? 'javascript:window.close();' : ''; } }
  ];
  const doc = { title: 'Logout - 関大LMS', body: { innerText: 'コース利用中に、別のコースへのアクセスがリクエストされました。 関大LMSの他のウインドウやタブをすべて閉じ、複数同時に開いて操作しないでください。 王 漢隆 さん、おつかれ様でした。 今回の利用時間は 00:00:06 でした。' }, querySelectorAll(selector) { if (selector === 'a[href]') return anchors; if (selector === 'td, div, p, span, li') return textNodes; return []; } };
