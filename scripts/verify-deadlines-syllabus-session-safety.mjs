...
const collectContextSource = extractFunction(source, 'collectContext');
...
const contentSandbox = {
  ...
  window: {
    location: { href: 'https://kulms.tl.kansai-u.ac.jp/webclass/?acs_=abc', origin: 'https://kulms.tl.kansai-u.ac.jp' },
    sessionStorage: {
...
const courseDoc = {
  querySelectorAll(selector) {
    ...
    return [{
      ...
      querySelectorAll(sel) {
        if (sel !== '.cl-contentsList_listGroupItem') return [];
        return [{ data: { title: '締切後の課題', type: '課題', availability: futureAvailability, detailHref: '/d3', historyHref: '', usage: '', usageCount: 0, href: '/d3' } }];
      }
    }];
  }
};
const parsedCourseUpcoming = contentSandbox.parseUpcomingFromCourse(courseDoc, '/course-a', { scheduleEntry: { title: '言語学 (2026-春学期-木曜日-1限-70340)', note: '締切が近い課題があります。', sortIndex: 4 } });
...
const parsedLoginUpcoming = contentSandbox.parseUpcomingFromCourse(loginLikeDoc, '/course-b', { scheduleEntry: { title: 'ヒューマンエージェントインタラクション＜M＞＜C＞ (2026-春学期-金曜日-2限-70399)', note: '締切が近い課題があります。', sortIndex: 2 } });
...
const report = { ok: true, checks: ['no-nonhome-home-fetch', 'explicit-course-visits-still-refresh-cache', 'retired-worker-upcoming-fetch-path', 'refresh-mode-side-effects-suppressed', 'course-upcoming-parser-still-valid', 'syllabus-direct-resolver-contract-preserved', 'active-docs-point-to-safe-refresh-phase'] };
