...
- Homepage announcements enrichment fetches `/webclass/information.php/`, parses the notification rows, and merges that full list with the current-page `homeNotices` preview before upcoming aggregation; duplicates are removed by notice identity.
- Homepage upcoming-deadline aggregation must use the full merged notice source before any visible preview is capped; only the rendered notice preview and rendered upcoming card list are truncated to five items.
- Homepage upcoming-deadline aggregation must stay inside homepage-safe sources only: current homepage DOM plus `/webclass/information.php/`. It must not crawl or prefetch course pages during automatic homepage enrichment.
- Homepage upcoming-deadline aggregation derives candidate items from notice titles, extracts due datetimes from the notice text, matches each notice back to a timetable course by shortened course title, prioritizes timetable entries already marked with `締切が近い課題があります`, then orders remaining items by unknown/no-usage before used, nearest due date, and finally title.
- If a timetable course is red-flagged on the homepage but no safe notice-derived task detail can be produced for that course, the redesign should add an honest fallback card that sends the user into the course to check the pending deadline there, instead of inventing a fake due datetime or hiding the course completely.
- Announcement-derived upcoming items may reuse known course-note context, but they must not fabricate `未利用` or other usage-state labels when the homepage-safe source does not actually provide usage evidence.
