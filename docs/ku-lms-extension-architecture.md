...
- After a same-tab direct syllabus jump proves a safe public detail URL, remember the safe detail URL by normalized course identity and reuse it when a later same-tab resolver attempt transiently fails after Back/history restore; do not use that memory to guess across unresolved first-time ambiguities.
- On supported KU-LMS home/course surfaces, browser Back/history restore must not leave syllabus chips relying on their raw public-search fallback anchors; repeated chip clicks after restore must re-enter the JS-controlled direct-detail / remembered-detail / search-fallback chain.
