...
- `その他のコース` row-level reminders should reuse the same red `締切が近い課題があります。` chip when the native homepage `.course-contents-info` field is present, so first-render parity matches timetable cards.
- Same-tab cache may still supplement the right-column `期限が近い課題` card with detailed other-course items, but row-level `その他のコース` chips must not depend on prior course visits.
- The homepage refresh button should refresh timetable red-flag rows and any `その他のコース` row that already exposes the native homepage reminder, while cache-only detailed items remain non-targets.
